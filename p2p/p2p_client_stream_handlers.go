...
func (client *Client) HandleReceiveConfigRequest(stream inet.Stream) {
	...
	writer.Write(append(config.Bytes(), '\b')) // Write config bytes
...
func (client *Client) HandleReceiveTransaction(stream inet.Stream) {
	...
	b, err := reader.ReadBytes('\b') // Read up to delimiter
	...
	b = bytes.Trim(b, "\b") // Trim delimiter
...
func (client *Client) HandleReceiveBestTransaction(stream inet.Stream) {
	...
	accountString, err := readWriter.ReadBytes('\b') // Read

	if err != nil { // Check for errors
		common.Logf("== P2P == error while reading req_best_tx stream: %s\n", err.Error()) // Log error
	}

	accountString = bytes.Trim(accountString, "\b") // Trim delimiter
	...
	if len(chain.Transactions) > 0 { // Check has txs
		readWriter.Write(append(chain.Transactions[len(chain.Transactions)-1].Hash.Bytes(), '\b')) // Write tx hash
	} else { // No txs
		readWriter.Write(append(common.NewHash(crypto.Sha3(nil)).Bytes(), '\b')) // Write nil hash
	}
...
func (client *Client) HandleReceiveNextTransactionRequest(stream inet.Stream) {
	...
	lastTxAccount, err := readWriter.ReadBytes('\b') // Read

	if err != nil { // Check for errors
		common.Logf("== P2P == error while reading req_next_tx stream: %s\n", err.Error()) // Log error
	}

	lastTxAccount = bytes.Trim(lastTxAccount, "\b") // Trim delimiter
	...
	if bytes.Equal(hash.Bytes(), common.NewHash(crypto.Sha3(nil)).Bytes()) { // Check is nil request
		readWriter.Write(append(accountChain.Transactions[0].Bytes(), '\b')) // Write genesis bytes
	...
	for x, transaction := range accountChain.Transactions { // Iterate through transactions
		if bytes.Equal(transaction.Hash.Bytes(), hash.Bytes()) { // Check hashes equal
			if len(accountChain.Transactions) == x+1 { // Check no next
				readWriter.Write(append(accountChain.Transactions[x].Bytes(), '\b')) // Write current transaction
			...
			readWriter.Write(append(accountChain.Transactions[x+1].Bytes(), '\b')) // Write next transaction
...
func (client *Client) HandleReceiveAllChainsRequest(stream inet.Stream) {
	...
	_, err = writer.Write(append([]byte(strings.Join(allLocalChains, "_")), '\b')) // Write all local chains
...
func (client *Client) HandleReceiveChainRequest(stream inet.Stream) {
	...
	addressBytes, err := readWriter.ReadBytes('\b') // Read up to delimiter

	if err != nil { // Check for errors
		common.Logf("== P2P == error while reading req_chain stream: %s\n", err.Error()) // Log error
	}

	addressBytes = bytes.Trim(addressBytes, "\b") // Trim delimiter
	...
	_, err = readWriter.Write(append(chain.Bytes(), '\b')) // Write chain bytes
...
func (client *Client) HandleReceiveAliveRequest(stream inet.Stream) {
	...
	_, err := writer.Write(append([]byte("despacito"), '\b')) // Write alive
