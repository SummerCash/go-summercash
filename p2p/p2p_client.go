...
func (client *Client) PublishTransaction(ctx context.Context, transaction *types.Transaction) error {
	...
	for _, peer := range peers { // Iterate through peers
		...
		_, err = writer.Write(append(transaction.Bytes(), '\b')) // Write message
...
func (client *Client) RequestBestTransaction(account common.Address, sampleSize uint) (common.Hash, error) {
	...
	responses, err := BroadcastDhtResult(ctx, client.Host, append([]byte(account.String()), '\b'), GetStreamHeaderProtocolPath(client.Network, RequestBestTransaction), client.Network, int(sampleSize)) // Broadcast, get result
...
func (client *Client) RequestNextTransaction(lastTransactionHash common.Hash, account common.Address, sampleSize uint) (*types.Transaction, error) {
	...
	responses, err := BroadcastDhtResult(ctx, client.Host, append([]byte(fmt.Sprintf("%s_%s", account.String(), lastTransactionHash.String())), '\b'), GetStreamHeaderProtocolPath(client.Network, RequestNextTransaction), client.Network, int(sampleSize)) // Broadcast, get result
...
func (client *Client) RequestAllChains(sampleSize uint) ([]string, error) {
	...
	responses, err := BroadcastDhtResult(ctx, client.Host, append([]byte("req_all_chains"), '\b'), GetStreamHeaderProtocolPath(client.Network, RequestAllChains), client.Network, int(sampleSize)) // Broadcast, get result
