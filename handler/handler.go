...
func handleConnection(conn net.Conn, isArchival bool) error {
	...
	case "chainRequ":
		...
		_, err = conn.Write(append(chain.Bytes(), byte('\f'))) // Write chain
	...
	case "cChainReq": // Check coordinationChain request
		...
		_, err = conn.Write(append(chainBytes, byte('\f'))) // Write chain
	...
	case "configReq":
		...
		_, err = conn.Write(append(configBytes, byte('\f'))) // Write chain
	...
	case "stateRequ":
		...
		_, err = conn.Write(append(state, byte('\f'))) // Write chain
