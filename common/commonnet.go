...
func SendBytes(b []byte, address string) error {
	...
	_, err = writer.Write(append(b, byte('\b'))) // Write data
...
func SendBytesResult(b []byte, address string) ([]byte, error) {
	...
	_, err = readWriter.Write(append(b, byte('\b'))) // Write data to connection
	...
	response, err := readWriter.ReadBytes('\b') // Read up to delimiter

	if err != nil { // Check for errors
		return nil, err // Return found error
	}

	return bytes.Trim(response, "\b"), nil // Return response
}
...
func ReadConnectionWaitAsyncNoTLS(conn net.Conn) ([]byte, error) {
	...
	readBytes, err := reader.ReadBytes('\b') // Read up to delimiter

	if err != nil { // Check for errors
		return nil, err // Return found error
	}

	return bytes.Trim(readBytes, "\b"), nil // Return read bytes w/trimmed delimiter
}
