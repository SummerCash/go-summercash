...
func SendBytes(b []byte, address string) error {
	...
	_, err = writer.Write(append(b, byte('\x99'))) // Write data
...
func SendBytesResult(b []byte, address string) ([]byte, error) {
	...
	_, err = readWriter.Write(append(b, byte('\x99'))) // Write data to connection
	...
	response, err := readWriter.ReadBytes('\x99') // Read up to delimiter

	if err != nil { // Check for errors
		return nil, err // Return found error
	}

	return bytes.Trim(response, "\x99"), nil // Return response
}
...
func ReadConnectionWaitAsyncNoTLS(conn net.Conn) ([]byte, error) {
	...
	readBytes, err := reader.ReadBytes('\x99') // Read up to delimiter

	if err != nil { // Check for errors
		return nil, err // Return found error
	}

	return bytes.Trim(readBytes, "\x99"), nil // Return read bytes w/trimmed delimiter
}
