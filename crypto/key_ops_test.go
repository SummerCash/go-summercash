...
func TestMarshalPublicKey(t *testing.T) {
	privateKey, err := ecdsa.GenerateKey(common.PreferredCurve(), rand.Reader) // Initialize private key
...
func TestUnmarshalPublicKey(t *testing.T) {
	...
	if publicKey.X != privateKey.PublicKey.X || publicKey.Y != privateKey.PublicKey.Y { // Check either x or y not equal
